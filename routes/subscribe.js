...
router.post('', function (req, res, next) {
    ...
    if (isValid){
        insertSubscribe(req.body['email'], req.body['name']).then(obj => {
            sendMail(req.body['name'], req.body['email']).then(res => {
              if (obj.subcribe){
                obj.subcribe.isSent = true;
                obj.subcribe.save();
              }
            }).catch(err => {
              console.log("Subscribe - sendMail: ",err);
            });
            sendMailToMe(req.body['name'], req.body['email']).then(res => res).catch(err => {
              console.log("Subscribe - sendMailToMe: ",err);
            })
